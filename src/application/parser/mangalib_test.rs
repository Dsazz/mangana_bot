...
mod tests {
    ...
    fn points_to_valid_extractor() {
        let parser = get_parser();
        let expected_extractor = &MangalibExtractor::new(Html::new_document());

        let box_extractor = parser.extractor(Html::new_document());
        let actual_extractor = box_extractor.as_any().downcast_ref::<MangalibExtractor>();
    ...
    fn validate_topic_site_with_topic_unknown_site() {
        ...
        let result = Mangalib::new().validate_topic_site(topic_with_unknown_site);
    ...
    fn validate_topic_site_with_topic_wrong_site() {
        ...
        let topic_with_wrong_site = Topic {
            id: 1,
            site_id: Site::Mangapoisk.id(),
            name: "Test name".to_string(),
            url_name: "test-url".to_string()
        };
        let result = parser.validate_topic_site(topic_with_wrong_site);
        assert!(result.is_err());
